...
# `content()` turns JSON into a nested R object
parsed <- content(news_return)

# Combine article records into a data frame using `bind_rows()`
article_info <- bind_rows(parsed$articles)
...
ggplot(
  ...
  geom_bar() + #default is stacked, while "dodge" makes side by side
  labs(
...
ggplot(
  ...
  geom_bar(position = "dodge") + #default is stacked, while "dodge" makes side by side
  labs(
    x = "Roller Coaster Scale",
    y = "Number of Roller Coasters",
    title = "Roller Coaster Type by Scale"
