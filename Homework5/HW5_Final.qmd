...
```
NOTE: using `content()` and `bind_rows()` so the nested source information remains 
a list column, as specified in the assignment; using `fromJSON()` would automatically 
simplify the nested structure into a data frame.
...
news_query <- function(subject, start_date, api_key) {
  ...
  # Combine article records into a data frame
  article_info <- bind_rows(parsed$articles)
